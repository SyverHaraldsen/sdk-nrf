...
#if DT_HAS_CHOSEN(nordic_lwm2m_carrier_partition)
#define NVS_PARTITION DT_CHOSEN(nordic_lwm2m_carrier_partition)
#else
#define NVS_PARTITION DT_NODELABEL(lwm2m_carrier_partition)
#endif

#define NVS_SECTOR_COUNT    (PARTITION_NODE_SIZE(NVS_PARTITION) / NVS_SECTOR_SIZE)
#define NVS_STORAGE_OFFSET  PARTITION_NODE_OFFSET(NVS_PARTITION)
#define NVS_FLASH_DEVICE    DEVICE_DT_GET(PARTITION_NODE_MTD(NVS_PARTITION))
